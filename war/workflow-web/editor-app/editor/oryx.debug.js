...
ORYX.Core.SVG.Label = Clazz.extend({
	...
	_checkFittingToReferencedElem: function() {
		try {
			...
			//if (/Firefox[\/\s](\d+\.\d+)/.app(navigator.userAgent) && new Number(RegExp.$1)>=3) {
				var newtspans = [];
...
//app string: abcdefghijklmnopqrstuvwxyz????????????????,.-#+ 1234567890?????ABCDEFGHIJKLMNOPQRSTUVWXYZ;:_'*???????????????!"????$%&/()=?[]{}|<>'~????`\^?????????@?????????????????
...
ORYX.Core.StencilSet.Rules = {
	...
	outgoingEdgeStencils: function(args) {
		...
		// app each edge, if it can connect to source
